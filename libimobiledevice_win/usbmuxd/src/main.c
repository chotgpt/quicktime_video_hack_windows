...
//循环创建目录
static int _loop_mkdir(const char* path)
{
	int len = strlen(path);
	char pathname[MAX_PATH];			//用于存储上级目录
	...
	if (_access(pathname, 0) == 0)	//若上级目录已存在
	{
		int i = 0;
		_mkdir(path);		//创建目标目录
	}
	...
	{
		_loop_mkdir(pathname);		//递归创建上级目录
		_mkdir(path);		//创建上级目录后创建目标目录
	}
...
#ifdef WIN32
static int ppoll(struct pollfd*fds, nfds_t nfds, int timeout)
{
...
static void usage()
{
	...
	printf("\n");
	printf("build by :    <" USBMUXD_PACKAGE_NAME "> data : <" __DATE__ ">\n");
}
...
int main(int argc, char *argv[])
{
	...
	usbmuxd_log(LL_NOTICE, "usbmuxd v%s starting up", USBMUXD_PACKAGE_VERSION);
	should_exit = 0;
	should_discover = 0;
