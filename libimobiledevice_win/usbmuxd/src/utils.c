 ...
 */
char * usbmuxd_stpcpy(char * s1, const char * s2)
{
	if (s1 == NULL || s2 == NULL)
		return NULL;
 ...
 */
char * usbmuxd_string_concat(const char *str, ...)
{
	size_t len;
	va_list args;
	char *s;
	...
	dest = usbmuxd_stpcpy(dest, str);

	va_start(args, str);
	s = va_arg(args, char *);
	while (s) {
		dest = usbmuxd_stpcpy(dest, s);
		s = va_arg(args, char *);
	}
	va_end(args);
...
int usbmuxd_buffer_read_from_filename(const char *filename, char **buffer, uint64_t *length)
{
	FILE *f;
	uint64_t size;
	...
	*buffer = (char*)malloc(sizeof(char)*(size+1));
	...
	if (fread(*buffer, sizeof(char), size, f) != size) {
		usbmuxd_log(LL_ERROR, "%s: ERROR: couldn't read %d bytes from %s", __FUNCTION__, (int)size, filename);
		free(*buffer);
		ret = 0;
		errno = EIO;
	}
...
int usbmuxd_buffer_write_to_filename(const char *filename, const char *buffer, uint64_t length)
{
	FILE *f;
...
int usbmuxd_plist_read_from_filename(plist_t *plist, const char *filename)
{
	char *buffer = NULL;
	uint64_t length;
	...
	if (!usbmuxd_buffer_read_from_filename(filename, &buffer, &length)) {
		return 0;
	}
...
int usbmuxd_plist_write_to_filename(plist_t plist, const char *filename, enum plist_format_t format)
{
	char *buffer = NULL;
	uint32_t length;
	...
	int res  = usbmuxd_buffer_write_to_filename(filename, buffer, length);
