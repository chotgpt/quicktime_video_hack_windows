...
#define HAVE_WINUSB	//这里设置使用winusb内核还是libusb内核

#if defined(HAVE_WINUSB)
#elif defined(HAVE_LIBUSB)
#elif defined(HAVE_IOKIT)
#ifdef WIN32
#error "windows not support HAVE_IOKIT"
#endif 
#else
#error "not set have mode!!  eg. #define HAVE_WINUSB or #define HAVE_LIBUSB or #define HAVE_IOKIT"
#endif
...
struct irecv_client_private {
	...
	unsigned int type;	//enum irecv_type
	unsigned int mode;
...
//get serial(dfu iboot)
static int irecv_copy_serial_with_path(const char* szPipePath, char* szSerial)
{
	char szPipePathBuffer[256];
	char* p;

	if (!szPipePath || !szSerial) return -1;
	szSerial[0] = '\0';

	strcpy_s(szPipePathBuffer, sizeof(szPipePathBuffer), szPipePath);
	p = szPipePathBuffer;
	while (p++) 
	{
	...
	if (!p || (sscanf(p, "\\usb#vid_%*04x&pid_%*04x#%s", szSerial) != 1) || (szSerial[0] == '\0')) 
	{
		return -2;
	}
...
//split serial
static void irecv_load_device_info_from_serial(irecv_client_t client, const char* serial)
{
	if (!client || !serial) {
		return;
	}
	...
	client->device_info.serial_string = strdup(serial);

	char* ptr;

	ptr = strstr(serial, "CPID:");
	if (ptr != NULL) {
		sscanf(ptr, "CPID:%x", &client->device_info.cpid);
	}

	ptr = strstr(serial, "CPRV:");
	if (ptr != NULL) {
		sscanf(ptr, "CPRV:%x", &client->device_info.cprv);
	}

	ptr = strstr(serial, "CPFM:");
	if (ptr != NULL) {
		sscanf(ptr, "CPFM:%x", &client->device_info.cpfm);
	}

	ptr = strstr(serial, "SCEP:");
	if (ptr != NULL) {
		sscanf(ptr, "SCEP:%x", &client->device_info.scep);
	}

	ptr = strstr(serial, "BDID:");
	if (ptr != NULL) {
		sscanf(ptr, "BDID:%x", &client->device_info.bdid);
	}

	ptr = strstr(serial, "ECID:");
	if (ptr != NULL) {
		sscanf(ptr, "ECID:%llx", &client->device_info.ecid);
	}

	ptr = strstr(serial, "IBFL:");
	if (ptr != NULL) {
...
//split descriptor
static void irecv_copy_nonce_with_descriptor(const char* descriptor, const char* tag, unsigned char** nonce, unsigned int* nonce_size)
{
...
//get serial and descriptor(adfu)
static void irecv_get_adfu_serial_descriptor(irecv_client_t client, char** serial, char** descriptor)
{
	struct AdfuDeviceInfo
	{
		unsigned short vid;         //01-02 vid
		unsigned short pid;         //03-04 pid
		DWORD Unknowm04_08;         //04-08 未知 000001A0
		char szDescriptor[0xFF];     //0x08  - 0x106(\0终止) NONC:XXXXXX SNON:XXXXXX
		char szSerial[0xFF];      //0x107 - 0x205(\0终止) SDOM:01 CPID:8120 CPRV:11 CPFM:03 SCEP:01 BDID:0E ECID:XXXXXX IBFL:3C SIKA:00 SRTG:[iBoot-7195.0.0.300.25]
	...
	*descriptor = NULL;
#if defined(HAVE_WINUSB)
	struct AdfuDeviceInfo adfuDeviceInfo;
	memset(&adfuDeviceInfo, 0, sizeof(adfuDeviceInfo));
	if (!DeviceIoControl(client->handle, 0x220004, NULL, 0, &adfuDeviceInfo, sizeof(adfuDeviceInfo), 0, NULL))
		return;
	*descriptor = malloc(strlen(adfuDeviceInfo.szDescriptor) + 1);
	strcpy(*descriptor, adfuDeviceInfo.szDescriptor);
	*serial = malloc(strlen(adfuDeviceInfo.szSerial) + 1);
	strcpy(*serial, adfuDeviceInfo.szSerial);
#elif defined(HAVE_LIBUSB)
...
static const GUID GUID_DEVINTERFACE_ADFU = { 0xB36f4137, 0xF4EF, 0x4BFC, {0xA2, 0x5A, 0xC2, 0x41, 0x07, 0x68, 0xEE, 0x37} };	//iPhone 14 Pro DFU Mode(System Device --> Apple RSM)
...
irecv_error_t mobiledevice_connect_get_adfu(irecv_client_t* client, unsigned long long ecid)
{
	int found = 0;
	...
	if (!_client) return IRECV_E_OUT_OF_MEMORY;
	memset(_client, 0, sizeof(struct irecv_client_private));

	/* get DFU paths */
	usbDevices = SetupDiGetClassDevs(&GUID_DEVINTERFACE_ADFU, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
	if (!usbDevices) return IRECV_E_UNKNOWN_ERROR;
	memset(&currentInterface, '\0', sizeof(SP_DEVICE_INTERFACE_DATA));
	currentInterface.cbSize = sizeof(SP_DEVICE_INTERFACE_DATA);
	for (i = 0; usbDevices && SetupDiEnumDeviceInterfaces(usbDevices, NULL, &GUID_DEVINTERFACE_ADFU, i, &currentInterface); i++) 
	{
		if(_client->szPipePath) 
			free(_client->szPipePath);
		_client->szPipePath = NULL;
		_client->handle = NULL;
		_client->type = IRECV_K_ADFU_TYPE;
		...
		if (!details) return IRECV_E_OUT_OF_MEMORY;
		...
		if (!result) return IRECV_E_OUT_OF_MEMORY;
		memcpy((void*)result, details->DevicePath, requiredSize - sizeof(DWORD));
		free(details);

		_client->szPipePath = result;
...
irecv_error_t mobiledevice_connect_get_dfu(irecv_client_t* client, unsigned long long ecid)
{
	int found = 0;
	SP_DEVICE_INTERFACE_DATA currentInterface;
	HDEVINFO usbDevices;
...
irecv_error_t mobiledevice_connect_get_iboot(irecv_client_t* client, unsigned long long ecid)
{
	int found = 0;
	SP_DEVICE_INTERFACE_DATA currentInterface;
	HDEVINFO usbDevices;
	...
	for (i = 0; usbDevices && SetupDiEnumDeviceInterfaces(usbDevices, NULL, &GUID_DEVINTERFACE_IBOOT, i, &currentInterface); i++) 
	{
		...
		_client->szPipePath = NULL;
		_client->handle = NULL;
		_client->type = IRECV_K_IBOOT_TYPE;
		DWORD requiredSize = 0;
		PSP_DEVICE_INTERFACE_DETAIL_DATA details;
		SetupDiGetDeviceInterfaceDetail(usbDevices, &currentInterface, NULL, 0, &requiredSize, NULL);
		details = (PSP_DEVICE_INTERFACE_DETAIL_DATA)malloc(requiredSize);
...
//type == enum irecv_type
irecv_error_t mobiledevice_connect_by_type(irecv_client_t* client, unsigned long long ecid, unsigned int type)
{
	int found = 0;
	SP_DEVICE_INTERFACE_DATA currentInterface;
	HDEVINFO usbDevices;
	...
	switch (_client->type)
	{
	case IRECV_K_DFU_TYPE:
		pGuid = &GUID_DEVINTERFACE_DFU;
		break;
	case IRECV_K_IBOOT_TYPE:
		pGuid = &GUID_DEVINTERFACE_IBOOT;
		break;
	case IRECV_K_ADFU_TYPE:
		pGuid = &GUID_DEVINTERFACE_ADFU;
		break;
	default:
		break;
	}
	if (!pGuid) return IRECV_E_UNKNOWN_ERROR;

	/* get DFU paths */
	usbDevices = SetupDiGetClassDevs(pGuid, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
	if (!usbDevices) return IRECV_E_UNKNOWN_ERROR;
	memset(&currentInterface, '\0', sizeof(SP_DEVICE_INTERFACE_DATA));
	currentInterface.cbSize = sizeof(SP_DEVICE_INTERFACE_DATA);
	for (i = 0; usbDevices && SetupDiEnumDeviceInterfaces(usbDevices, NULL, pGuid, i, &currentInterface); i++)
	{
		if (_client->szPipePath)
			free(_client->szPipePath);
		_client->szPipePath = NULL;
		...
		if (_client->type == IRECV_K_DFU_TYPE
			|| _client->type == IRECV_K_IBOOT_TYPE)
		{
			char szSerial[255];
			char szDescriptor[255];
			if (irecv_copy_serial_with_path(_client->szPipePath, szSerial) != 0)
			{
				mobiledevice_closepipes(_client);
				continue;
			}
			irecv_load_device_info_from_serial(_client, szSerial);
			irecv_get_string_descriptor_ascii(_client, 1, (unsigned char*)szDescriptor, sizeof(szDescriptor));
			irecv_copy_nonce_with_descriptor(szDescriptor, "NONC", &_client->device_info.ap_nonce, &_client->device_info.ap_nonce_size);
			irecv_copy_nonce_with_descriptor(szDescriptor, "SNON", &_client->device_info.sep_nonce, &_client->device_info.sep_nonce_size);
		}
		else if (_client->type == IRECV_K_ADFU_TYPE)
		{
			char* serial = NULL;
			char* descriptor = NULL;
			irecv_get_adfu_serial_descriptor(_client, &serial, &descriptor);
			if (!serial || !descriptor)
			{
				mobiledevice_closepipes(_client);
				continue;
			}
			irecv_load_device_info_from_serial(_client, serial);
			irecv_copy_nonce_with_descriptor(descriptor, "NONC", &_client->device_info.ap_nonce, &_client->device_info.ap_nonce_size);
			irecv_copy_nonce_with_descriptor(descriptor, "SNON", &_client->device_info.sep_nonce, &_client->device_info.sep_nonce_size);
		}
...
irecv_error_t mobiledevice_connect(irecv_client_t* client, unsigned long long ecid) 
{
	//if (IRECV_E_SUCCESS == mobiledevice_connect_get_adfu(client, ecid)) return IRECV_E_SUCCESS;
	//if (IRECV_E_SUCCESS == mobiledevice_connect_get_dfu(client, ecid)) return IRECV_E_SUCCESS;
	//if (IRECV_E_SUCCESS == mobiledevice_connect_get_iboot(client, ecid)) return IRECV_E_SUCCESS;
	if (IRECV_E_SUCCESS == mobiledevice_connect_by_type(client, ecid, IRECV_K_ADFU_TYPE)) return IRECV_E_SUCCESS;
	if (IRECV_E_SUCCESS == mobiledevice_connect_by_type(client, ecid, IRECV_K_DFU_TYPE)) return IRECV_E_SUCCESS;
	if (IRECV_E_SUCCESS == mobiledevice_connect_by_type(client, ecid, IRECV_K_IBOOT_TYPE)) return IRECV_E_SUCCESS;
	return IRECV_E_NO_DEVICE;
}

irecv_error_t mobiledevice_openpipes(irecv_client_t client) 
{
	if(!client->szPipePath) 
	{
		return IRECV_E_UNABLE_TO_CONNECT;
	}
	client->handle = CreateFile(client->szPipePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
	if (INVALID_HANDLE_VALUE == client->handle)
	{
		return IRECV_E_UNABLE_TO_CONNECT;
	}
	...
	{
	case IRECV_K_DFU_TYPE:
		if (strncmp(client->szPipePath, "\\\\?\\usb#vid_05ac&pid_", 21) == 0)
			sscanf(client->szPipePath + 21, "%x#", &client->mode);
		break;
	case IRECV_K_IBOOT_TYPE:
		if (strncmp(client->szPipePath, "\\\\?\\usb#vid_05ac&pid_", 21) == 0)
			sscanf(client->szPipePath + 21, "%x#", &client->mode);
		break;
	case IRECV_K_ADFU_TYPE:
		//   \\?\kis#vid_05ac&pid_0001#6&1bc5df2d&1&01#{b36f4137-f4ef-4bfc-a25a-c2410768ee37}\1a0
		if (strncmp(client->szPipePath, "\\\\?\\kis#vid_05ac&pid_", 21) == 0)
			sscanf(client->szPipePath + 21, "%x#", &client->mode);
		break;
	default:
		client->mode = 0;
...
IRECV_API irecv_error_t irecv_open_with_ecid(irecv_client_t* pclient, unsigned long long ecid) {
	...
	if (ret == IRECV_E_SUCCESS) {
		...
		if ((client->mode != IRECV_K_DFU_MODE) && (client->mode != IRECV_K_WTF_MODE) && (client->mode != IRECV_K_ADFU_MODE))
		{
			error = irecv_usb_set_interface(client, 0, 0);
			if (client->mode > IRECV_K_RECOVERY_MODE_2) {
				error = irecv_usb_set_interface(client, 1, 1);
...
static void* _irecv_handle_device_add(void *userdata)
{
	...
	irecv_load_device_info_from_serial(&client_loc, serial_str);
...
static void _irecv_handle_device_remove(struct irecv_usb_device_info *devinfo)
{
	...
	free(devinfo);
		...
		static int LIBUSB_CALL _irecv_usb_hotplug_cb(libusb_context* ctx, libusb_device* device, libusb_hotplug_event event, void* user_data)
		{
...
#if defined(HAVE_WINUSB)
		free(client->szPipePath);
		client->szPipePath = NULL;
