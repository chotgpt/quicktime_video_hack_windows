...
#endif
#endif
	__config_dir = usbmuxd_string_concat(base_config_dir, DIR_SEP_S, CONFIG_DIR, NULL);
...
static int internal_set_value(const char *config_file, const char *key, plist_t value)
{
	...
	usbmuxd_plist_read_from_filename(&config, config_file);
	if (!config) {
	...
	int res = usbmuxd_plist_write_to_filename(config, config_file, PLIST_FORMAT_XML);
...
static int config_set_value(const char *key, plist_t value)
{
	...
	config_file = usbmuxd_string_concat(config_path, DIR_SEP_S, CONFIG_FILE, NULL);
...
static int internal_get_value(const char* config_file, const char *key, plist_t *value)
{
	*value = NULL;
	...
	plist_t config = NULL;
	if (usbmuxd_plist_read_from_filename(&config, config_file)) {
...
int config_has_device_record(const char *udid)
{
	...
	/* build file path */
	const char *config_path = config_get_config_dir();
	char *device_record_file = usbmuxd_string_concat(config_path, DIR_SEP_S, udid, CONFIG_EXT, NULL);
...
int config_set_device_record(const char *udid, char* record_data, uint64_t record_size)
{
	...
	/* store file */
	if (!usbmuxd_plist_write_to_filename(plist, device_record_file, PLIST_FORMAT_XML)) {
...
int config_get_device_record(const char *udid, char **record_data, uint64_t *record_size)
{
	...
	/* read file */
	usbmuxd_buffer_read_from_filename(device_record_file, record_data, record_size);
	if (!*record_data) {
...
static int config_device_record_get_value(const char *udid, const char *key, plist_t *value)
{
	...
	config_path = config_get_config_dir();
	config_file = usbmuxd_string_concat(config_path, DIR_SEP_S, udid, CONFIG_EXT, NULL);
