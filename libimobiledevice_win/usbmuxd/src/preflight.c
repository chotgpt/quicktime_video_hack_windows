...
static void lockdownd_set_untrusted_host_buid(lockdownd_client_t lockdown)
{
	...
	usbmuxd_log(LL_DEBUG, "%s: Setting UntrustedHostBUID to %s", __FUNCTION__, system_buid);
...
static void np_callback(const char* notification, void* userdata)
{
	...
	if (strcmp(notification, "com.apple.mobile.lockdown.request_pair") == 0) {
		usbmuxd_log(LL_INFO, "%s: user trusted this computer on device %s, pairing now", __FUNCTION__, _dev->udid);
		lerr = lockdownd_client_new(dev, &lockdown, "usbmuxd");
		if (lerr != LOCKDOWN_E_SUCCESS) {
			usbmuxd_log(LL_ERROR, "%s: ERROR: Could not connect to lockdownd on device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
		...
		if (lerr != LOCKDOWN_E_SUCCESS) {
			usbmuxd_log(LL_ERROR, "%s: ERROR: Pair failed for device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
...
static void* preflight_worker_handle_device_add(void* userdata)
{
	...
	if (!_dev)
	{
		usbmuxd_log(LL_FATAL, "%s: malloc fail.", __FUNCTION__);
		return NULL;
	}
	...
	usbmuxd_log(LL_INFO, "%s: Starting preflight on device %s...", __FUNCTION__, _dev->udid);
...
retry:
	...
	if (!type) {
		usbmuxd_log(LL_ERROR, "%s: ERROR: Could not get lockdownd type from device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
		goto leave;
	}
	...
	if (strcmp(type, "com.apple.mobile.lockdown") != 0) {
		...
		free(type);
		usbmuxd_log(LL_INFO, "%s: Finished preflight on device %s", __FUNCTION__, _dev->udid);
	...
	if (config_has_device_record(dev->udid)) {
		...
		if (host_id)
			free(host_id);
		if (lerr == LOCKDOWN_E_SUCCESS) {
			usbmuxd_log(LL_INFO, "%s: StartSession success for device %s", __FUNCTION__, _dev->udid);
			usbmuxd_log(LL_INFO, "%s: Finished preflight on device %s", __FUNCTION__, _dev->udid);
			client_device_add(info);
			goto leave;
		}

		usbmuxd_log(LL_INFO, "%s: StartSession failed on device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
	} else {
		lerr = LOCKDOWN_E_INVALID_HOST_ID;
	}
	switch (lerr) {
	case LOCKDOWN_E_INVALID_HOST_ID:
		usbmuxd_log(LL_INFO, "%s: Device %s is not paired with this host.", __FUNCTION__, _dev->udid);
		break;
	case LOCKDOWN_E_SSL_ERROR:
		usbmuxd_log(LL_ERROR, "%s: The stored pair record for device %s is invalid. Removing.", __FUNCTION__, _dev->udid);
		if (config_remove_device_record(_dev->udid) == 0) {
			lockdownd_client_free(lockdown);
			lockdown = NULL;
			goto retry;
		} else {
			usbmuxd_log(LL_ERROR, "%s: Could not remove pair record for device %s", __FUNCTION__, _dev->udid);
		}
		break;
	default:
		is_device_paired = 1;
	...
	if (lerr != LOCKDOWN_E_SUCCESS) {
		usbmuxd_log(LL_ERROR, "%s: ERROR: Could not get ProductVersion from device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
		goto leave;
	}
	...
	if (!version_str) {
		usbmuxd_log(LL_ERROR, "%s: Could not get ProductVersion string from device %s handle %d", __FUNCTION__, _dev->udid, (int)(long)_dev->conn_data);
		goto leave;
	}
	...
	if (version_major >= 7) {
		/* iOS 7.0 and later */
		usbmuxd_log(LL_INFO, "%s: Found ProductVersion %s device %s", __FUNCTION__, version_str, _dev->udid);
		...
		if (!is_device_paired) {
			if (lockdownd_pair(lockdown, NULL) == LOCKDOWN_E_SUCCESS) {
				...
				usbmuxd_log(LL_INFO, "%s: Pair success for device %s", __FUNCTION__, _dev->udid);
				usbmuxd_log(LL_INFO, "%s: Finished preflight on device %s", __FUNCTION__, _dev->udid);
				client_device_add(info);
				goto leave;
			}
		...
		if (lerr != LOCKDOWN_E_SUCCESS) {
			...
			usbmuxd_log(LL_INFO, "%s: ERROR: Could not start insecure_notification_proxy on %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
			client_device_add(info);
			goto leave;
		}
		...
		usbmuxd_log(LL_INFO, "%s: Waiting for user to trust this computer on device %s", __FUNCTION__, _dev->udid);
		...
		usbmuxd_log(LL_INFO, "%s: Finished waiting for notification from device %s, is_device_connected %d", __FUNCTION__, _dev->udid, cbdata.is_device_connected);
		...
		if (lerr != LOCKDOWN_E_SUCCESS) {
			if (lerr == LOCKDOWN_E_PASSWORD_PROTECTED) {
				usbmuxd_log(LL_INFO, "%s: Device %s is locked with a passcode. Cannot pair.", __FUNCTION__, _dev->udid);
				/* TODO send notification to user's desktop */
			} else {
				usbmuxd_log(LL_ERROR, "%s: ERROR: Pair failed for device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
			}
		...
		free(host_id);
		if (lerr != LOCKDOWN_E_SUCCESS) {
			usbmuxd_log(LL_ERROR, "%s: ERROR StartSession failed on device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
			goto leave;
		}

		lerr = lockdownd_validate_pair(lockdown, NULL);
		if (lerr != LOCKDOWN_E_SUCCESS) {
			usbmuxd_log(LL_ERROR, "%s: ERROR: ValidatePair failed for device %s, lockdown error %d", __FUNCTION__, _dev->udid, lerr);
			goto leave;
		}
