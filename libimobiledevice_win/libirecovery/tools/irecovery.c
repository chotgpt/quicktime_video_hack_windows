...
static void buffer_read_from_filename(const char *filename, char **buffer, uint64_t *length) {
	...
	*buffer = (char*)malloc(sizeof(char)*((long)size+1));
	fread(*buffer, sizeof(char), (long)size, f);
	fclose(f);
...
static void print_device_info(irecv_client_t client)
{
	...
	printf("Recovery Mode Environment:\n");
	char* value = NULL;
	irecv_getenv(client, "build-version", &value);
	printf("iBoot build-version=%s\n", (value) ? value : "(unknown)");
	free(value);
	value = NULL;
}
...
int main(int argc, char* argv[]) {
	...
	switch (action) {
		...
		case kStartShell:
				...
				init_shell(client);
			#else
				fprintf(stderr, "sorry, not support shell\n");
			#endif
			break;
		case kSendScript:
